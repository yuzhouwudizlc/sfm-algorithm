...
def extract_features(image_names):
    ...
    for image_name in image_names:
        ...
        #使用sift.detectAndCompute函数抽取图像的关键点和对应的特征描述符
        #这里使用了灰度图像作为输入
    ...
    return np.array(key_points_for_all, dtype=object), np.array(descriptor_for_all, dtype=object), np.array(colors_for_all, dtype=object)
...
def get_matched_points(p1, p2, matches):
    '''
        src_pts 和 dst_pts 分别包含了源图像和目标图像中匹配对的特征点坐标，可以用于后续的操作和计算
    '''
...
def fig_v1(structure):
    ...
    mlab.points3d(structure[:, 0], structure[:, 1], structure[:, 2], mode = 'point', name = 'dinosaur', colormap='jet' )
...
def main():
    ...
    '''
        extract_features 提取特征点
        key_points_for_all 提取的特征点
        descriptor_for_all 特征描述符
        colors_for_all 颜色信息
